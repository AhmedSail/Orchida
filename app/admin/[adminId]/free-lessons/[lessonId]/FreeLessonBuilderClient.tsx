...
import {
  ...
  getFreeLessonByIdAction,
} from "@/app/actions/free-lessons";
import {
  Plus, Trash2, Video, Type, Image as ImageIcon, FileText,
  Link as LinkIcon, GripVertical, ArrowRight, Loader2, Save,
  Heading1, Heading2, Quote, AlertTriangle, Minus, Pencil,
  Layout, ExternalLink, Upload, X,
} from "lucide-react";
...
export default function FreeLessonBuilderClient({ adminId, lessonId, initialLesson }: Props) {
  const [lesson, setLesson] = useState<any>(initialLesson);
  const [isAddingField, setIsAddingField] = useState(false);
  const [isEditingTitle, setIsEditingTitle] = useState(false);
  const [editMainTitle, setEditMainTitle] = useState(initialLesson.mainTitle);
  const [editSubTitle, setEditSubTitle] = useState(initialLesson.subTitle || "");
  ...
  async function refreshLesson() {
    ...
    if (res.success && res.data) {
      setLesson(res.data);
      setEditMainTitle(res.data.mainTitle);
  ...
  const handleDeleteField = async (fieldId: string) => {
    ...
    if (result.isConfirmed) {
      ...
      if (res.success) { toast.success("تم الحذف"); refreshLesson(); }
    }
  };
  ...
  const handleEditFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadFile(file, p => setEditUploadProgress(p));
  ...
  const handleSaveField = async (fieldId: string) => {
    ...
    const res = await updateFreeLessonFieldAction(fieldId, { content: editFieldContent });
    if (res.success) {
  ...
  const handleSaveOrder = async () => {
    ...
    const updatedFields = lesson.fields.map((f: any, i: number) => ({ id: f.id, order: i + 1 }));
    const res = await reorderFreeLessonFieldsAction(updatedFields);
    if (res.success) { toast.success("تم حفظ الترتيب"); refreshLesson(); }
    ...
    <div className="max-w-4xl mx-auto py-10 px-6" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-12 bg-white p-6 rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            {isEditingTitle ? (
              <div className="space-y-2">
                <input value={editMainTitle} onChange={e => setEditMainTitle(e.target.value)}
                  ...
                  placeholder="عنوان الدرس" autoFocus />
                ...
                <div className="flex gap-2">
                  <button onClick={handleSaveTitle} disabled={isSavingTitle}
                    className="bg-primary text-white text-xs font-black px-4 py-1.5 rounded-lg hover:bg-primary/90 transition-all">
                    {isSavingTitle ? "حفظ..." : "حفظ"}
                  </button>
                  <button onClick={() => setIsEditingTitle(false)}
                    className="text-xs font-bold text-zinc-400 px-3 py-1.5 rounded-lg hover:bg-zinc-100 transition-all">
                    إلغاء
                  </button>
                </div>
              </div>
              ...
              <>
                ...
                <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mt-1">إدارة محتوى الدرس المجاني</p>
              </>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href={`/free-lessons/${lessonId}`} target="_blank"
            className="bg-emerald-50 text-emerald-600 px-5 py-3 rounded-2xl font-black flex items-center gap-2 hover:bg-emerald-500 hover:text-white transition-all shadow-sm border border-emerald-100">
            <ExternalLink className="w-4 h-4" /> معاينة
          </Link>
          <Link href={`/admin/${adminId}/free-lessons`}
      ...
      <div className="space-y-6">
        <Reorder.Group axis="y" values={lesson.fields || []}
          onReorder={newFields => setLesson({ ...lesson, fields: newFields })}
          className="space-y-4">
                ...
                {editingFieldId === field.id ? (
                  <div className="space-y-4">
                      ...
                      {["image", "file", "video"].includes(field.fieldType) && (
                        <div className="relative">
                          ...
                          <button onClick={() => editFileInputRef.current?.click()}
                            ...
                            {editUploadProgress !== null ? `${editUploadProgress}%` : "رفع مباشر"}
                          </button>
                        </div>
                      )}
                    </div>
                    <textarea value={editFieldContent} onChange={e => setEditFieldContent(e.target.value)}
                      ...
                      placeholder={field.fieldType === "text" ? "اكتب المحتوى النصي..." : field.fieldType === "video" ? "رابط يوتيوب..." : "الرابط..."}
                      autoFocus />
                    ...
                    <div className="flex justify-end gap-2 mt-4">
                      <button onClick={() => setEditingFieldId(null)}
                        className="px-6 py-2 font-bold text-zinc-400 hover:bg-zinc-50 rounded-xl transition-all">إلغاء</button>
                  ...
                  <div className="flex items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => { setEditingFieldId(field.id); setEditFieldContent(field.content); }}
        ...
        {isAddingField ? (
          <div className="animate-in zoom-in-95 duration-200">
            <AddFieldForm lessonId={lessonId}
              onCancel={() => setIsAddingField(false)}
              onSuccess={() => { setIsAddingField(false); refreshLesson(); }} />
          </div>
        ) : (
...
function AddFieldForm({ lessonId, onCancel, onSuccess }: { lessonId: string; onCancel: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    const res = await addFreeLessonFieldAction({
      lessonId, fieldType: type, content: type === "divider" ? "---" : content,
    });
    setIsSubmitting(false);
    if (res.success) { onSuccess(); toast.success("تم إضافة المحتوى"); }
      ...
      {type !== "divider" && (
        <div className="space-y-4">
            ...
            {["image", "file", "video"].includes(type) && (
              <div className="relative">
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={uploadProgress !== null}
                  ...
                  {uploadProgress !== null ? `${uploadProgress}%` : `رفع ${type === "image" ? "صورة" : type === "video" ? "فيديو" : "ملف"}`}
                </button>
              </div>
            )}
          </div>
          <textarea
            ...
            placeholder={type === "text" ? "اكتب المحتوى..." : type === "video" ? "رابط يوتيوب..." : type === "link" ? "https://example.com" : "الرابط..."}

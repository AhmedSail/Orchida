...
export default function AiPricingTable({
  initialPricing,
}: {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await upsertAiPricingAction({
        ...
        duration:
          formData.serviceType === "video" ? formData.duration : undefined,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/40 backdrop-blur-md p-6 rounded-3xl border border-white/50 shadow-xl shadow-slate-200/50">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1 pr-1">
            تحكم في تكلفة الكريدت لكل جودة ونوع وموديل
          </p>
        </div>
      ...
      <AnimatePresence>
        {activeTab === "pricing" && isAdding && (
          ...
          >
            <form
              onSubmit={handleSave}
              className="bg-white p-8 rounded-3xl border border-slate-200 shadow-2xl shadow-slate-200/50 space-y-6"
            >

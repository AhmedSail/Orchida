...
const AcceptApplicationDialog = ({
  ...
  const handleAccept = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`/api/course-applications/${applicationId}`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        Swal.fire({
          title: "تم القبول!",
          text: `تم تحويل الطالب ${studentName} إلى الشعبة المختارة بنجاح.`,
          icon: "success",
      ...
      <DialogContent className="sm:max-w-[450px] rounded-3xl" dir="rtl">
        <DialogHeader className="text-right">
          ...
          <DialogDescription className="text-zinc-500 font-medium pt-2">
            أنت بصدد قبول طلب الطالب <span className="text-primary font-bold">{studentName}</span> في دورة <span className="font-bold">{courseTitle}</span>. 
            {attendanceType && (
              <div className="mt-1 flex items-center gap-1">
                رغبة الطالب: 
                <span className={`font-black ${attendanceType === 'online' ? 'text-blue-600' : attendanceType === 'hybrid' ? 'text-purple-600' : 'text-orange-600'}`}>
                  {attendanceType === 'online' ? 'أونلاين' : attendanceType === 'hybrid' ? 'مدمج' : 'وجاهي'}
                </span>
              </div>
            )}
            يرجى تحديد الشعبة المناسبة له.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 bg-red-50 rounded-2xl border border-red-100 text-red-600 gap-2">
                ...
                <p className="text-[10px] opacity-70">يجب إنشاء شعبة "مفتوحة" أولاً لتتمكن من قبول الطلاب.</p>
              </div>
            )}
          </div>
        </div>

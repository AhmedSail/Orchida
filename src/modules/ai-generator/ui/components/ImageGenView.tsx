...
export default function ImageGenView() {
  ...
  const cost = (() => {
    if (provider === "Grok") {
      // Grok has no resolution tier — match by provider only
      const match = pricingRules.find(
        (r) =>
          r.serviceType === "image" &&
          r.provider.toLowerCase() === "grok",
      );
      return match ? match.credits : 5;
    }
              ...
              : orientation.includes("9:16")
                ...
                : orientation.includes("2:3")
                  ...
                  : orientation.includes("3:2")
                    ...
                    : "1:1"
            : aspectRatio,
        orientation, // send full orientation string for backend parsing
        outputFormat,
            ...
            {isGenerating ? (
              ...
              <div className={`grid w-full gap-2.5 ${
                ...
                {resultImageUrls.map((url, idx) => (
                  <div
                    key={idx}
                    onClick={() => { setLightboxUrl(url); setLightboxIdx(idx); }}
      ...
      {lightboxUrl && (
        ...
        >
          ...
          {resultImageUrls.length > 1 && (
            <button
              onClick={e => { e.stopPropagation(); const prev = (lightboxIdx - 1 + resultImageUrls.length) % resultImageUrls.length; setLightboxIdx(prev); setLightboxUrl(resultImageUrls[prev]); }}
          ...
          {resultImageUrls.length > 1 && (
            <button
              onClick={e => { e.stopPropagation(); const next = (lightboxIdx + 1) % resultImageUrls.length; setLightboxIdx(next); setLightboxUrl(resultImageUrls[next]); }}
          ...
          {resultImageUrls.length > 1 && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
              {resultImageUrls.map((u, i) => (
                <button
                  key={i}
                  onClick={e => { e.stopPropagation(); setLightboxIdx(i); setLightboxUrl(u); }}
